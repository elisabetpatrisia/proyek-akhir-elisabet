...
<body>
	...
	<section>
		...
		<div style="margin: 40px">
			<div class="container events events-1" id="inner-page-title">
				<!-- TITLE & DESCRIPTION -->
				<div class="spe-title">
					<h2> <span>Tabel Kuota</span> </h2>
					<a class="link-btn" data-toggle="modal" data-target=".tambahModal" style="margin: 5px;">Tambah Kuota</a>
				</div>
				<div class="col-md-3" style="float: left; height: 350px">
					...
					<div class="input-field" >
						<div class="row">
							<a class="link-btn carikuota"  value="">Cari</a>						
						</div>
					</div>
				</div>
				
					<div class="container col-md-9" id="kuotapenumpang">
					...
					<div class="modal-dialog">
						<div style="height: 270px" class="modal-content">
							<div class="modal-header">
								...
								<h4 class="modal-titlle">Tambah Kuota</h4>
							</div>
							<div class="modal-body">
								<form method="POST" action="<?php echo base_url();?>c_penumpang/tambahKuota">
									<span>Rute</span>
									<select name="rute" required >
										<option disabled>Rute</option>		
										<?php
											foreach ($rute as $ta) {  
													foreach ($terminal as $key) {
														if ($ta->id_terminal_keberangkatan == $key->id) {
															$terminalkeberangkatan = $key->nama_terminal;
														}

														if ($ta->id_terminal_kedatangan == $key->id) {
															$terminalkedatangan = $key->nama_terminal;
														}
													}
													?>

													<option value="<?= $ta->id_rute?>"><?= $terminalkeberangkatan?> - <?= $terminalkedatangan?> (<?= $ta->jam_keberangkatan?>) </option>
												<?php  }
										?>
									</select>
									<br>
									<div class="col-md-6" >
										<span>Kuota</span>
										<input required type="number" name="kuota" class="form-control" placeholder="Jumlah Kuota" max="30" >	
									</div>
									<div class="col-md-6">
										<span>Tanggal</span>
	...
	$(document).on('click','.carikuota', function(){
		var tanggal = document.getElementById('tanggal').value;

		$.ajax({
			url:'<?php echo base_url()?>c_penumpang/carikuotapenumpang/'+tanggal,
			type:'POST',
			data:{'tanggal' : tanggal},
			success:function(data){
				$('#kuotapenumpang').html(data);
			}
		});
	});
	</script>
